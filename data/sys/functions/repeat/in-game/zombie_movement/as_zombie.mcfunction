...
# ゾンビの位置からサバイバーが16ブロック以内にいる時に実行
    execute as @s at @s if entity @p[team=Iruru.survivor,distance=..16,gamemode=survival] run function sys:repeat/in-game/zombie_movement/delta_pos_y

# ゾンビの位置からサバイバーが17~32ブロック以内にいる時に実行
    execute as @s[tag=Iruru.standing] at @s rotated ~ 0 anchored eyes if entity @p[team=Iruru.survivor,distance=17..32,gamemode=survival] unless block ^ ^ ^1 air run function sys:repeat/in-game/zombie_movement/movement/upward/1block
