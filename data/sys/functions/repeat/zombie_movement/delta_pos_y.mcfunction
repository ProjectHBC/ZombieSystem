...
# Y座標に関する計算
    ...
    # プレイヤーとゾンビのY座標の差を計算し、ゾンビ側の計算用スコアボードに格納
        scoreboard players operation @s iruru.d_pos_y -= @a[team=Iruru.survivor,distance=..16,sort=nearest,gamemode=survival] iruru.pos_y
...
# Y座標の差を利用した動き
    ...
    # プレイヤーとゾンビのY座標が同じときかつ2ブロック下が#can_placeのときに実行
        execute if score @s iruru.d_pos_y matches 0 as @s[nbt={OnGround:1b}] at @s rotated ~ 0 if entity @a[team=Iruru.survivor,distance=..16,sort=nearest,gamemode=survival] if block ^ ^-0.5 ^0.25 #minecraft:can_place if block ^ ^-1.5 ^0.25 #minecraft:can_place run function sys:repeat/zombie_movement/movement/forward/
