...
# 指定座標にカスタムスポナーを設置
    ...
    # pontako
        execute positioned 229 70 -496 run function sys:event/custom_spawner/set
        execute positioned 223 70 -528 run function sys:event/custom_spawner/set
        execute positioned 197 70 -498 run function sys:event/custom_spawner/set
        execute positioned 194 70 -533 run function sys:event/custom_spawner/set
        execute positioned 245 70 -498 run function sys:event/custom_spawner/set
        execute positioned 231 75 -540 run function sys:event/custom_spawner/set
        execute positioned 209 75 -488 run function sys:event/custom_spawner/set
        execute positioned 265 70 -497 run function sys:event/custom_spawner/set
...
# メッセージ
    # 設置が完了
        execute if score $cs_count iruru.cs_timer matches 8 run tellraw @s {"text": "カスタムスポナーを指定座標に設置しました。\n既にほかのカスタムスポナーが設置されている場合、不具合がある可能性があります。","color": "green"}
    # 設置が未完了、エラーとして処理
        execute if score $cs_count iruru.cs_timer matches ..7 run tellraw @s {"text": "カスタムスポナーを指定座標に設置できませんでした。\n指定座標がロードされてない可能性があります。\n再実行する前にカスタムスポナーを削除することを推奨します。","color": "red"}
